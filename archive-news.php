    ...
    <section class="archive container">
        ...
        <div class="archive-wrapper">
            <ul class="archive-list">
                ...
                <!-- archive.phpはそもそも新着情報のデータをもってこれるのでWP＿Queryじゃなくていいい -->
                <?php if (have_posts()) : ?>
            ...
            <ul class="sidebar-list">
                ...
                <div class="category">
                    ...
                    <ul>
                        <?php
                        // カスタムタクソノミー「news_category」のすべてのタームを取得
                        $news_categories = get_terms(array(
                            'taxonomy' => 'news_category',
                            'hide_empty' => false, // 空のタームも含める場合は false にする
                        ));

                        if (!empty($news_categories) && !is_wp_error($news_categories)) {
                            foreach ($news_categories as $news_category) {
                                $term_link = get_term_link($news_category);

                                // リンクが有効な場合のみ出力
                                if (!is_wp_error($term_link)) {
                                    echo '<li><a href="' . esc_url($term_link) . '">' . esc_html($news_category->name) . '</a> (' . esc_html($news_category->count) . ')</li>';
                                }
                            }
                        }
                        ?>
